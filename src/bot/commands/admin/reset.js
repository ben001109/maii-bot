...
import { deletePlayer, deleteAllPlayersAllGuilds, deletePlayersByGuild, getPlayer, getOrCreatePlayer } from '../../../services/playerService.js';
import { logger } from '../../utils/Logging.js';
import { redis } from '../../../redis/redisClient.js'; // 請確認路徑正確
import { isAdmin } from '../../utils/adminControl.js';
import { createRequire } from 'node:module';
import { sendAdminMessage, sendSuccess, sendError, sendResetResult } from '../../utils/ReplyUtils.js';
...
export default {
  ...
  async execute(interaction) {
    ...
    if (!interaction.memberPermissions.has(PermissionFlagsBits.Administrator)) {
      return sendAdminMessage(interaction, '❌ 僅限管理員操作');
    }
    ...
    try {
      ...
      if (subcommand === 'user') {
        ...
        if (!target) {
          return sendAdminMessage(interaction, '❌ 請提供要重置的目標用戶。');
        }
        await deletePlayer(target.id);
        logger.warn(`[RESET][USER] ${adminId} 重置了 ${target.id} 的玩家資料`);
        const player = await getOrCreatePlayer(target.id); // 確保重新初始化
        return sendResetResult(interaction, `✅ 已成功重置 <@${target.id}> 的帳號資料！`, player);
      }
      ...
      if (subcommand === 'guild') {
        ...
        return sendResetResult(interaction, `⚠️ 已成功重置本伺服器所有玩家資料（ID: ${guildId}）`, interaction.user);
      }

      // reset all（超級/指定管理員）
      if (subcommand === 'all') {
        if (!(SUPER_ADMINS.includes(adminId) || await isAdmin(guildId, adminId, config))) {
          return sendAdminMessage(interaction, '❌ 你不是超級管理員或本伺服器指定管理員，不能進行全服重置！');
        }
        await deleteAllPlayersAllGuilds();
        logger.error(`[RESET][ALL] 管理員 ${adminId} 重置了全服所有玩家資料！`);
        return sendResetResult(interaction, '⚠️ 已清空全服所有玩家資料！', interaction.user);
      }

      return sendAdminMessage(interaction, '❌ 不支援的重置類型。');

    } catch (err) {
      const errorMsg = `[RESET][${subcommand}] 發生錯誤：${err instanceof Error ? err.message : JSON.stringify(err)}\n${err.stack ?? ''}`;
      logger.error(errorMsg);
      return sendError(interaction, `❌ 重置失敗，請稍後再試。\n\`\`\`\n${err.message ?? err}\n\`\`\``);
    }
  }
};